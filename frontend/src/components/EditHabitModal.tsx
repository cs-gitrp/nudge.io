...
export const EditHabitModal: React.FC<EditHabitModalProps> = ({ habit, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await modifyHabit(habit.id, name.trim(), description.trim() || undefined);
      onClose();
    } catch (err: unknown) {
      ...
      <div className="relative w-full max-w-md transform overflow-hidden rounded-2xl border border-zinc-200 bg-white p-6 shadow-2xl transition-all animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-zinc-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-zinc-900">Edit Habit</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div>
            <label htmlFor="edit-habit-name" className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">
              Habit Name *
            </label>
            <input
              id="edit-habit-name"
          ...
          <div>
            <label htmlFor="edit-habit-description" className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">
